...
import actors.SimilarTicketFinder._
...
class ActorController @Inject() (
  ...
  val director = system.actorOf(Director.props(sparkContext, sparkSession), "director")
      ...
      case Some(ticket) ⇒ {
        ...
        val result = for {
          ...
          similarityResult ← (similarityFinder ? Similar(ticket)).map {
            case array: Array[TicketSimilarityResult] ⇒ array
            case a @ e ⇒ Array[TicketSimilarityResult]()
          }
        } yield similarityResult
        result.flatMap { content ⇒
          val similarTicket = content.sortBy(t ⇒ -t.similarity)
