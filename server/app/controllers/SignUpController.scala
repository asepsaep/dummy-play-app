...
class SignUpController @Inject() (
  ...
  @Named("account-service-related-actor") signUpActor: ActorRef,
  eventBus: EventBus,
  ...
  eventBus.subscribe(signUpActor, classOf[CredentialsSignUpEvent])
  eventBus.subscribe(signUpActor, classOf[EmailConfirmedEvent])
  ...
  def submit = silhouette.UnsecuredAction.async { implicit request =>
    if (appMode.isProd) {
      ...
      formFoldHelper(SignUpForm.form.bindFromRequest)
    }
  }

  def formFoldHelper(form: Form[SignUpForm.Data])(implicit request: Request[AnyContent]) = {
    form.fold(
